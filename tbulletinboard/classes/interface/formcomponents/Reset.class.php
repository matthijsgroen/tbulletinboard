<?php
	...
	class FormReset extends FormButton {
		...
		function FormReset($caption, $title, $description, $name, $onclick = "") {
			$this->FormButton($caption, $title, $description, $name, $onclick);
			$this->randomName = uniqID("submit");
		}

		function getInput() {
			$result = '';
			$result .= '<span class="button">'."\n";
			$onClick = $this->privateVars["onclick"]."; this.form.submitValue.value='".$this->identifier."'";

			$result .= '<button type="reset" onclick="'.$onClick.
