<?php
	...
	class FormNumberField extends FormTextField {
		 ...
		 */
		function FormNumberField($name, $title, $description, $maxlength, $required = false, $disabled = false, $prefix = "", $postfix = "") {
			$this->FormTextField($name, $title, $description, $maxlength, $required, $disabled, $prefix, $postfix);
			$this->privateVars['type'] = "number";
		...
		function hasValidValue($postData = null) {
			...
			//is_Numeric($this->form->getValue($this->privateVars['name'], $this->privateVars['type'])); // staat een . ook toe
			//return preg_match('/^-?\d+$/', $s); // doet ook negatieve waarden
			return preg_match('/^\d+$/', $s);
		}

		function getErrorMessage() {
			return ivMLGS("library", 21, "Geen geldig geheel getal");
		}
